...
import { ink, nonWhite } from "@/lib/color";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">All teams with logos and photos. Click a team to manage its players.</p>
    ...
    >
      <div className="grid md:grid-cols-2">
        <Link
          to="/teams/$teamId"
          params={{ teamId: t.id }}
          className="flex flex-col justify-center gap-4 p-8 transition hover:bg-secondary/30"
        ...
        >
          ...
          <div>
            ...
            <h2 className="mt-1 text-3xl font-black" style={{ color: ink(t.primary_color) }}>{t.name}</h2>
            <p className="mt-1 text-xs uppercase tracking-wider text-muted-foreground">{t.abbreviation}</p>
          </div>
          {t.description && <p className="text-sm text-muted-foreground">{t.description}</p>}
          <span className="text-xs font-bold" style={{ color: ink(t.primary_color) }}>View roster →</span>
        </Link>
...
function TeamCard({ team }: { team: Team }) {
  return (
    <Link to="/teams/$teamId" params={{ teamId: team.id }} className="block rounded-2xl border bg-card p-4 transition hover:shadow-md hover:-translate-y-0.5">
      <div className="flex items-center gap-3">
        {team.logo_url ? <img src={team.logo_url} alt="" className="h-12 w-12 rounded-lg border bg-white object-contain" /> : <div className="h-12 w-12 rounded-lg" style={{ background: ink(team.primary_color) }} />}
        <div className="min-w-0">
          <h3 className="truncate font-bold" style={{ color: ink(team.primary_color) }}>{team.name}</h3>
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from("teams").insert({
        name, abbreviation: abbr || name.slice(0, 3).toUpperCase(), primary_color: nonWhite(color), logo_url, photo_url,
      });
      if (error) throw error;
