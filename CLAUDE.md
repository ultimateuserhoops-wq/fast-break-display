...
- **AI calls:** the Worker and browser make **no** direct AI calls. AI features (Gemini for footage analysis + schedule advice; Anthropic for Telegram tips) exist **only in the local `gateway/` Bun process**, which calls `api.anthropic.com` and `generativelanguage.googleapis.com` **directly** with keys from its own env. The workspace-wide "route AI through the Kie.ai proxy" rule does **not** apply to this repo — the gateway runs on a local operator machine (not on Workers/APAC), so direct provider calls are fine here.
...
- **Open RLS:** `game_state` (and peers) use `FOR ALL TO authenticated USING (true)` — any signed-in operator can write any court. Shared login = no real per-user identity; `audit.ts` stamps an operator name as a soft audit trail. Treat RLS as NOT a security boundary.
...
- Do NOT call Anthropic/Gemini from the Worker or browser — AI belongs in the local `gateway/`.
